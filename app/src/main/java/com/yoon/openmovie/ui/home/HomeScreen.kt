...
fun HomeScreen(
    ...
    Box(modifier = Modifier){
        ...
        AnimatedVisibility(visible = !state.isLoading && state.error == null) {
            //  Box 내부에서 BoxWithConstraints를 사용하여
            // 부모 컨테이너의 제약 조건에 접근
            // fillMaxSize() 수정자를 사용하여 BoxWithConstraints가
            // 가능한 최대 크기를 차지하도록 설정
            // BoxWithConstraints는 내부에서 부모의 최대 높이와 너비에 접근할 수 있게 해줌
            // 이를 통해 자식 컴포저블의 크기를 동적으로 조정할 수 있음
